...
def the_system():
    ...
    system.add_component(
        ...
        fm_name="frun",
        targets=["CA", "CB"],
        failure_effects={"v_flow_out": 3, "flow_out_max": 3},
...
def test_system(the_system):
    ...
    assert the_system.comp["T1"].flow_in_max.value() == -1
    assert the_system.comp["T1"].flow_available_out.value() is True
    assert the_system.comp["T2"].flow_in_max.value() == -1
    assert the_system.comp["T2"].flow_available_out.value() is True
    assert the_system.comp["T3"].flow_in_max.value() == -1
    assert the_system.comp["T3"].flow_available_out.value() is True
    assert the_system.comp["T4"].flow_in_max.value() == -1
    assert the_system.comp["T4"].flow_available_out.value() is True
    ...
    for cname in ["T1", "T3", "T4"]:
        assert the_system.comp[cname].v_flow_out.value() == 4
