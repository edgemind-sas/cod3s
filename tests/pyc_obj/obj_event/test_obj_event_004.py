...
def the_system():
    ...
    event_list = [
        {
            "name": "top_event4",  
            "cond" : [[{"attr": "occ", "obj": "C1__frun", "ope": "==", "value": True}]],
            "inner_logic" :all,
            "outer_logic":any,
            "tempo_occ" :0,
            "tempo_not_occ" : 0,
    ...
    system.add_targets([{"name": "top_event4", "enabled": True}])

    system.add_indicator(
        component="top_event4",
        attr_type="ST",
        attr_name="occ",
        stats=["mean"],
    )
...
def test_system(the_system):
    ...
    assert "top_event4" in the_system.comp
    comp_event = the_system.comp["top_event4"]

    assert comp_event.name() == "top_event4"
    ...
    assert "top_event4_tempo" in the_system.comp
    comp_event = the_system.comp["top_event4_tempo"]

    assert comp_event.name() == "top_event4_tempo"
    assert comp_event.className() == "ObjEvent"
    assert comp_event.metadata == {}
    automaton = comp_event.automata_d["ev"]
    assert len(automaton.transitions) == 2
    ...
    assert "top_event4_not_occ" in the_system.indicators.keys()
    ind_not_occ = the_system.indicators["top_event4_not_occ"]
    ...
    assert "top_event4_occ" in the_system.indicators.keys()
    ind_occ = the_system.indicators["top_event4_occ"]
    ...
    assert "top_event4_tempo_not_occ" in the_system.indicators.keys()
    ind_not_occ = the_system.indicators["top_event4_tempo_not_occ"]
    val = ind_not_occ.values["values"].to_list()
    assert ind_not_occ.instants == schedule
    assert val[0] == True
    assert val[1] == True
    ...
    assert "top_event4_tempo_occ" in the_system.indicators.keys()
    ind_occ = the_system.indicators["top_event4_tempo_occ"]
    val = ind_occ.values["values"].to_list()
    assert ind_occ.instants == schedule
    assert val[0] == False
    assert val[1] == False
