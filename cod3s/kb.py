...
class ComponentTemplate(ObjCOD3S):
    ...
    class_name_bkd: Optional[str] = Field(
        None,
    ...
    def create_instance(self, name, **kwargs):
        ...
        if self.attributes:
            for attr_name, attr_template in self.attributes.items():
                ...
                # Réinitialiser la valeur courante pour éviter qu'elle ne soit automatiquement
                # définie à la valeur par défaut (pour les tests de deep copy)
    ...
    def __str__(self):
        ...
        if attr_count > 0:
            ...
            for name, attribute in self.attributes.items():
                ...
                # Si la valeur actuelle est différente de la valeur par défaut, on la colore en blanc
                if (
