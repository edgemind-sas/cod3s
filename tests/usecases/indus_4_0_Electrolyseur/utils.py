...
def compare_csv_files(current_dir, tolerance=1e-3, subset=None):
    ...
    assert (
        not missing_in_result
    ), f"Error: The following files are missing in 'result': {missing_in_result}"
    assert (
        not missing_in_reference
    ), f"Error: The following files are missing in 'reference': {missing_in_reference}"
    ...
    for file_name in identical_files:
        ...
        try:
            ...
            for index in range(max(len(results_df), len(references_df))):
                ...
                ref_row = (
                    references_df.iloc[index] if index < len(references_df) else None
                )
                        ...
                        diff.append(
                            {
                                "Index": index,
                                "Reference": (
                                    ref_row[columns_to_include].to_dict()
                                    if columns_to_include
                                    else ref_row.to_dict()
                                ),
                                "Result": (
                                    result_row[columns_to_include].to_dict()
                                    if columns_to_include
                                    else result_row.to_dict()
                                ),
                            }
                        )
                    ...
                    diff.append(
                        {
                            ...
                            "Reference": (
                                ref_row[columns_to_include].to_dict()
                                if columns_to_include
                                else "Row missing in references"
                            ),
    ...
    if all_differences:
        raise AssertionError(
            f"Differences found. See the following files for details: {', '.join(all_differences)}"
        )
