...
def the_system():
    ...
    system.add_targets([{"name": "top_event6", "enabled": True}])

    system.add_indicator(
        component="top_event6",
        attr_type="ST",
        attr_name="occ",
        stats=["mean"],
    )
...
def test_system(the_system):
    ...
    assert "top_event6" in the_system.comp
    comp_event = the_system.comp["top_event6"]

    assert comp_event.name() == "top_event6"
    ...
    assert "top_event6_occ" in the_system.indicators.keys()
    ind_occ = the_system.indicators["top_event6_occ"]
    ...
    assert "top_event6_not_occ" in the_system.indicators.keys()
    ind_not_occ = the_system.indicators["top_event6_not_occ"]
